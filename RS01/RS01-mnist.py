...
mnist = scipy.io.loadmat(
    "/Users/wangqiyu/Documents/RS_study/RS01/mldata/mnist-original.mat"
)
...
train_x, test_x, train_y, test_y = train_test_split(
    data, mnist["label"], test_size=0.25, random_state=33
)
...
train_ss_x = ss.fit_transform(
    train_x
)  # 不仅计算数据的均值和方差，还会基于计算出来的均值和方差来转换数据，从而把数据转换成标准的正态分布。⚠️这里可以先fit，再transform。
test_ss_x = ss.transform(
    test_x
)  # ⚠️这里用训练数据的均值和方差来转换。它只进行转换，只是把数据转换成标准的正态分布
